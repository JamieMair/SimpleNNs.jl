Base.@kwdef struct Conv{DT<:Real, KSIZE<:Union{InferSize,NTuple{<:Any, Int}}, K<:Union{InferSize,Int}, T<:Function, B, KD, KS<:NTuple{KD, Int}} <: AbstractLayer
    out_channels::Int
    use_bias::Val{B} = Val(true)
    parameter_type::Val{DT} = Val(Float32)
    in_size::KSIZE = InferSize()
    in_channels::K = InferSize()
    kernel_size::KS
...
function inputsize(layer::Conv)
    layer.in_size isa InferSize && error("Conv layer inputs should be set to a number.")
...
function reconstruct_layer(layer::Conv, previous_layer_size::NTuple{D, Int}, previous_datatype) where {D}
    if layer.in_channels isa InferSize || layer.in_size isa InferSize
