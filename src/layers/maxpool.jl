...
function reconstruct_layer(layer::MaxPool, previous_layer_size, current_datatype)
    if layer.stride isa Infer || layer.input_size isa Infer || layer.output_size isa Infer || layer.DT isa Infer
        ...
        expected_output_size = map((in_dim, kernel_size, str)->_conv_dim_size(in_dim, kernel_size, 0, str), input_size, layer.pool_size, stride)
        ...
        datatype = ifelse(layer.DT isa Infer, Val(current_datatype), layer.datatype)
        return MaxPool(layer.pool_size, stride, input_size, expected_output_size, datatype)
    else
        return layer
    end
end
